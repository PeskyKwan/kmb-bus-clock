...
PersistentApproach primaryApproach;ApproachMotion motion[trackedBuses];EtaPositionTracker positionTracker;unsigned long lastPositionPoll=0;int positionCode=0,positionRows=0;uint32_t positionFrameGen=0;
...
void resetMotions(){primaryApproach.reset();for(auto&m:motion)m.reset();markerCount=0;markerPainted=false;}
void drawMap();
void mapGrid();
...
void updateAnimation(){
 ...
 time_t now=time(nullptr);if(!activeRoad.valid||activeRoad.approachCount<2){for(auto&m:motion)m.reset();restoreMarker();return;}float estimated[trackedBuses]={};bool fresh=activeRoad.gen==cfg.gen&&animationFresh(WiFi.status()==WL_CONNECTED,etaCode,now,dataStamp,etaEpoch);bool valid[trackedBuses]={fresh&&positionTracker.estimate(now,etaEpoch,estimated[0]),fresh&&positionTracker.estimate(1,now,estimated[1])};bool primaryVisible=primaryApproach.update(fresh,now,etaEpoch,activeRoad.meters,valid[0],estimated[0]);motion[0].update(primaryVisible,primaryApproach.fraction,seconds,true);motion[1].update(valid[1],estimated[1],seconds);
