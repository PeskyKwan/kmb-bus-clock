...
int etaCode=0;time_t etaEpoch=0,dataStamp=0,etaEpoch2=0,dataStamp2=0;bool etaScheduled=false,etaScheduled2=false;unsigned long lastPoll=0,lastRender=0,lastStatus=0,lastTouch=0;int failures=0;
float calibration[6];bool calibrated=false;int calStep=0;float rawCal[3][2];bool held=false;
int touchX=0,touchY=0;
PersistentApproach primaryApproach,secondaryApproach;ApproachMotion motion[trackedBuses];EtaPositionTracker positionTracker;EtaAvailabilityGate etaAvailability;unsigned long lastPositionPoll=0;int positionCode=0,positionRows=0;uint32_t positionFrameGen=0;
bool mapActualRoad=false,markerPainted=false;
int mapX[3],mapY[3],markerCount=0,markerX[trackedBuses]={},markerY[trackedBuses]={},markerAlpha[trackedBuses]={};bool markerSleeping[trackedBuses]={};
unsigned long lastAnimation=0;
void resetMotions(){primaryApproach.reset();secondaryApproach.reset();etaAvailability.reset();etaEpoch2=dataStamp2=0;etaScheduled=etaScheduled2=false;for(auto&m:motion)m.reset();markerCount=0;markerPainted=false;for(bool&s:markerSleeping)s=false;}
void drawMap();
void mapGrid();
...
void updateTheme(int mode=-1){
 ...
 if(dark!=nightMode){nightMode=dark;needsDraw=true;markerPainted=false;markerCount=0;}
...
bool api(const String&path,DynamicJsonDocument&out){for(int attempt=0;attempt<3;attempt++){WiFiClientSecure tls;tls.setCACert(kmbCA);tls.setHandshakeTimeout(12);tls.setTimeout(7);HTTPClient h;h.setConnectTimeout(6000);h.setTimeout(7000);h.useHTTP10(true);if(!h.begin(tls,"https://data.etabus.gov.hk/v1/transport/kmb/"+path))return false;int code=h.GET();char tlsError[1];int tlsCode=code<0?tls.lastError(tlsError,sizeof(tlsError)):0;apiTlsDiagnostic=tlsCode;if(code<0&&(code==-1||tlsCode==tlsConnectionEof)&&attempt<2){h.end();apiTlsRetries++;delay(500*(attempt+1));continue;}bool ok=false;if(code==200&&h.getSize()<65536){DeserializationError err;if(path.startsWith("route-stop/")){StaticJsonDocument<160> filter;filter["data"][0]["stop"]=true;filter["data"][0]["seq"]=true;err=deserializeJson(out,h.getStream(),DeserializationOption::Filter(filter));}else err=deserializeJson(out,h.getStream());apiDiagnostic=(int)err.code();ok=!err;}else apiDiagnostic=code;h.end();return ok;}return false;}
#include "route_stop_fetch.inc"
#include "position_fetch.inc"
// Read-only USB evidence. Stream rows without a large JSON allocation during TLS.
// The worker owns pendingFrame while a request is in flight; never read it then.
void emitPosition(){
 const time_t now=time(nullptr);bool busy=requestBusy;
 Serial.printf("{\"event\":\"position\",\"now\":%lld,\"target\":%lld,\"targetStamp\":%lld,\"etaCode\":%d,\"targetFresh\":%s,\"pollAgeMs\":%lu,\"busy\":%s,\"paths\":[",(long long)now,(long long)etaEpoch,(long long)dataStamp,etaCode,animationFresh(WiFi.status()==WL_CONNECTED,etaCode,now,dataStamp,etaEpoch)?"true":"false",lastPositionPoll?millis()-lastPositionPoll:0,busy?"true":"false");
 for(int i=0;i<trackedBuses;i++){if(i)Serial.print(',');Serial.printf("{\"state\":\"%s\",\"samples\":%d}",positionTracker.status(i,now,i?0:etaEpoch),positionTracker.samples(i));}Serial.print(']');Serial.printf(",\"http\":%d,\"tls\":%d,\"frameCurrent\":%s",positionHttpDiagnostic,positionTlsDiagnostic,positionFrameGen==cfg.gen?"true":"false");
 if(!busy&&positionFrameGen==cfg.gen){Serial.printf(",\"generated\":%lld,\"meters\":%.6f,\"anchors\":[",(long long)pendingFrame.generated,pendingFrame.layout.meters);for(int i=0;i<pendingFrame.layout.count;i++){if(i)Serial.print(',');Serial.printf("{\"seq\":%d,\"fraction\":%.8f,\"rows\":[",pendingFrame.layout.anchors[i].seq,pendingFrame.layout.anchors[i].fraction);for(int j=0;j<pendingFrame.counts[i];j++){if(j)Serial.print(',');auto r=pendingFrame.rows[i][j];Serial.printf("[%lld,%lld,%s,%d]",(long long)r.eta,(long long)r.stamp,r.scheduled?"true":"false",r.rank);}Serial.print("]}");}Serial.print(']');}Serial.println('}');
}
void worker(void*){Request req;uint32_t validated=0;for(;;){if(xQueueReceive(requests,&req,portMAX_DELAY)!=pdTRUE)continue;Result result={req.c.gen,-1,0,0,0,0,0,false,false};if(req.kind==1){result.kind=1;result.code=prepareRoad(req.c)?1:0;xQueueSend(results,&result,portMAX_DELAY);continue;}if(req.kind==2){result.kind=2;result.code=fetchPosition(req)?1:0;xQueueSend(results,&result,portMAX_DELAY);continue;}DynamicJsonDocument d(8192);
 if(validated!=req.c.gen){bool found=false;cachedRoadStops.clear();bool fetched=fetchRouteStops(req.c.route,req.c.bound,req.c.service,[&](const char*id,int seq){if(!strcmp(id,req.c.stop)&&seq==req.c.seq)found=true;if(seq<req.c.seq){WindowStop stop;if(cachedStop(id,stop))cachedRoadStops.push_back({stop,seq});}return cachedRoadStops.size()<256;});if(!fetched||!found){result.code=-2;xQueueSend(results,&result,portMAX_DELAY);continue;}std::sort(cachedRoadStops.begin(),cachedRoadStops.end(),[](const RoadStopCandidate&a,const RoadStopCandidate&b){return a.seq<b.seq;});if(cachedRoadStops.size()>=maxMapStops)cachedRoadStops.erase(cachedRoadStops.begin(),cachedRoadStops.end()-(maxMapStops-1));roadStopsGen=req.c.gen;validated=req.c.gen;d.clear();}
 ...
 else{for(JsonObject row:d["data"].as<JsonArray>()){if(String(row["route"]|"")!=req.c.route||String(row["dir"]|"")!=String(req.c.bound)||row["service_type"].as<int>()!=req.c.service||row["seq"].as<int>()!=req.c.seq)continue;time_t stamp=parseISO(row["data_timestamp"]|""),eta=parseISO(row["eta"]|"");if(!stamp||abs((long)(now-stamp))>120){result.code=-3;continue;}if(eta>=now-30&&eta<=now+10800&&(result.eta==0||eta<result.eta)){result.eta=eta;result.stamp=stamp;result.scheduled=scheduledEtaRemark(row["rmk_en"]|"",row["rmk_tc"]|"");result.code=2;}}}}
 if(result.code==2){time_t now=time(nullptr);for(JsonObject row:d["data"].as<JsonArray>()){if(String(row["route"]|"")!=req.c.route||String(row["dir"]|"")!=String(req.c.bound)||row["service_type"].as<int>()!=req.c.service||row["seq"].as<int>()!=req.c.seq)continue;if(considerFollowingEta(now,result.eta,parseISO(row["eta"]|""),parseISO(row["data_timestamp"]|""),result.eta2,result.stamp2))result.scheduled2=scheduledEtaRemark(row["rmk_en"]|"",row["rmk_tc"]|"");}}
...
void drawSleepSigns(int x,int y,int alpha,unsigned long tick){int phase=(tick/650)%2;bool right=x<=280;int bx=constrain(x+(right?11:-18),136,300),by=constrain(y-18+phase,38,161);auto pixel=[&](int px,int py){lcd.drawPixel(px,py,blend565(INK,lcd.pixelColor(px,py),alpha));};auto z=[&](int zx,int zy,int w,int h){for(int i=0;i<=w;i++){pixel(zx+i,zy);pixel(zx+i,zy+h);}for(int i=0;i<=h;i++)pixel(zx+w-i*w/h,zy+i);};z(bx,by+7,4,4);z(bx+(right?5:-1),by,6,6);}
void updateAnimation(){
 const unsigned long tick=millis();const float seconds=lastAnimation?(tick-lastAnimation)/1000.0f:.1f;lastAnimation=tick;
 time_t now=time(nullptr);if(!activeRoad.valid||activeRoad.approachCount<2){for(auto&m:motion)m.reset();restoreMarker();return;}float estimated[trackedBuses]={};bool fresh=activeRoad.gen==cfg.gen&&animationFresh(WiFi.status()==WL_CONNECTED,etaCode,now,dataStamp,etaEpoch);bool valid[trackedBuses]={fresh&&positionTracker.estimate(now,etaEpoch,estimated[0]),fresh&&positionTracker.estimate(1,now,estimated[1])};bool fresh2=activeRoad.gen==cfg.gen&&animationFresh(WiFi.status()==WL_CONNECTED,etaCode,now,dataStamp2,etaEpoch2);alignIllustratedPair(primaryApproach,secondaryApproach,now,fresh,etaEpoch,fresh2,etaEpoch2);bool primaryVisible=primaryApproach.update(fresh,now,etaEpoch,activeRoad.meters,valid[0],estimated[0]);bool secondVisible=secondaryApproach.update(fresh2,now,etaEpoch2,activeRoad.meters,fresh2&&valid[1]&&positionTracker.targetEta(1)==etaEpoch2,estimated[1]);motion[0].update(primaryVisible,primaryApproach.fraction,seconds,true);motion[1].update(secondVisible,secondaryApproach.fraction,seconds,true);
 int nextCount=0,nextX[trackedBuses]={},nextY[trackedBuses]={},nextAlpha[trackedBuses]={};bool nextSleeping[trackedBuses]={};const int count=activeRoad.approachCount;auto point=[&](int i,int axis)->float{return axis?activeRoad.points[i].y:activeRoad.points[i].x;};
 for(int bus=0;bus<trackedBuses;bus++){if(!motion[bus].visible)continue;float x=point(0,0),y=point(0,1);for(int i=1;i<count;i++){float a=activeRoad.pointFractions[i-1],b=activeRoad.pointFractions[i];if(motion[bus].fraction<=b||i==count-1){float u=b>a?constrain((motion[bus].fraction-a)/(b-a),0.f,1.f):0;x=point(i-1,0)+(point(i,0)-point(i-1,0))*u;y=point(i-1,1)+(point(i,1)-point(i-1,1))*u;break;}}int bob=motion[bus].fraction<.995f?((tick/650+bus)%2):0;nextX[nextCount]=constrain((int)roundf(x),145,296);nextY[nextCount]=constrain((int)roundf(y)+bob,48,161);nextAlpha[nextCount]=roundf(motion[bus].opacity*15);nextSleeping[nextCount]=standbyIllustration(bus?etaScheduled2:etaScheduled,bus?secondaryApproach.mode:primaryApproach.mode,motion[bus].fraction);nextCount++;}
 if(nextCount==2)separateMarkerCenters(nextX[0],nextY[0],nextX[1],nextY[1]);
 bool unchanged=markerPainted&&markerCount==nextCount;for(int i=0;i<nextCount&&unchanged;i++)unchanged=markerX[i]==nextX[i]&&markerY[i]==nextY[i]&&markerAlpha[i]==nextAlpha[i]&&markerSleeping[i]==nextSleeping[i];if(unchanged)return;if(markerPainted)restoreMarker();int sleep=-1;for(int i=0;i<nextCount;i++){markerX[i]=nextX[i];markerY[i]=nextY[i];markerAlpha[i]=nextAlpha[i];markerSleeping[i]=nextSleeping[i];drawBusMarker(markerX[i],markerY[i],markerAlpha[i]);if(markerSleeping[i]&&(sleep<0||markerX[i]>markerX[sleep]))sleep=i;}if(sleep>=0)drawSleepSigns(markerX[sleep],markerY[sleep],markerAlpha[sleep],tick);markerCount=nextCount;markerPainted=nextCount>0;
}
...
void loop(){web.handleClient();pollNative();pollWifiSetup();while(Serial.available()){char ch=Serial.read();if(ch=='\n'){DynamicJsonDocument d(4096);if(!deserializeJson(d,serialLine)){if(String(d["cmd"]|"")=="state")emitState();else if(String(d["cmd"]|"")=="position")emitPosition();else if(String(d["cmd"]|"")=="wifi")wifiCommand(d);else if(String(d["cmd"]|"")=="settings")nativeDebug(d);else if(!applyConfig(d,true)){lastMessage="設定無效，未儲存";emitState();}}serialLine="";}else if(serialLine.length()<4000)serialLine+=ch;else serialLine="";}
 ...
 Result result;if(xQueueReceive(results,&result,0)==pdTRUE){requestBusy=false;if(result.kind==1){if(result.gen==cfg.gen){roadTriedGen=cfg.gen;roadAttemptAt=millis();activeRoad=pendingRoad;needsDraw=true;}}else if(result.kind==2){if(result.gen==cfg.gen){positionFrameGen=result.gen;positionCode=result.code;positionRows=0;if(result.code)for(int i=0;i<pendingFrame.layout.count;i++)positionRows+=pendingFrame.counts[i];if(result.code)positionTracker.accept(pendingFrame,time(nullptr),etaEpoch);}}else if(result.gen==cfg.gen){etaCode=result.code;etaEpoch=result.eta;dataStamp=result.stamp;etaEpoch2=result.eta2;dataStamp2=result.stamp2;etaScheduled=result.scheduled;etaScheduled2=result.scheduled2;if(etaAvailability.update(etaCode)){resetMotions();positionTracker.reset();positionCode=positionRows=0;}failures=etaCode<0?min(failures+1,4):0;lastMessage=etaCode==2?"已連線，正在顯示真實九巴 ETA":etaCode==1?"已連線，九巴暫時沒有到站預報":etaCode==-2?"路線或車站未能核實，正在重試":"到站資料未能更新，正在重試";needsDraw=true;emitState();}}
