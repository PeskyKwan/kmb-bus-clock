...
struct Request{Config c;int kind;EtaLayout layout;};struct Result{uint32_t gen;int code;time_t eta,stamp;int kind;time_t eta2,stamp2;};
...
int etaCode=0;time_t etaEpoch=0,dataStamp=0,etaEpoch2=0,dataStamp2=0;unsigned long lastPoll=0,lastRender=0,lastStatus=0,lastTouch=0;int failures=0;
float calibration[6];bool calibrated=false;int calStep=0;float rawCal[3][2];bool held=false;
int touchX=0,touchY=0;
PersistentApproach primaryApproach,secondaryApproach;ApproachMotion motion[trackedBuses];EtaPositionTracker positionTracker;unsigned long lastPositionPoll=0;int positionCode=0,positionRows=0;uint32_t positionFrameGen=0;
bool mapActualRoad=false,markerPainted=false;
int mapX[3],mapY[3],markerCount=0,markerX[trackedBuses]={},markerY[trackedBuses]={},markerAlpha[trackedBuses]={};
unsigned long lastAnimation=0;
void resetMotions(){primaryApproach.reset();secondaryApproach.reset();etaEpoch2=dataStamp2=0;for(auto&m:motion)m.reset();markerCount=0;markerPainted=false;}
void drawMap();
void mapGrid();
...
void worker(void*){Request req;uint32_t validated=0;for(;;){if(xQueueReceive(requests,&req,portMAX_DELAY)!=pdTRUE)continue;Result result={req.c.gen,-1,0,0};if(req.kind==1){result.kind=1;result.code=prepareRoad(req.c)?1:0;xQueueSend(results,&result,portMAX_DELAY);continue;}if(req.kind==2){result.kind=2;result.code=fetchPosition(req)?1:0;xQueueSend(results,&result,portMAX_DELAY);continue;}DynamicJsonDocument d(8192);
 ...
 if(result.code==2){time_t now=time(nullptr);for(JsonObject row:d["data"].as<JsonArray>()){if(String(row["route"]|"")!=req.c.route||String(row["dir"]|"")!=String(req.c.bound)||row["service_type"].as<int>()!=req.c.service||row["seq"].as<int>()!=req.c.seq)continue;considerFollowingEta(now,result.eta,parseISO(row["eta"]|""),parseISO(row["data_timestamp"]|""),result.eta2,result.stamp2);}}
...
void updateAnimation(){
 ...
 int nextCount=0,nextX[trackedBuses]={},nextY[trackedBuses]={},nextAlpha[trackedBuses]={};const int count=activeRoad.approachCount;auto point=[&](int i,int axis)->float{return axis?activeRoad.points[i].y:activeRoad.points[i].x;};
 for(int bus=0;bus<trackedBuses;bus++){if(!motion[bus].visible)continue;float x=point(0,0),y=point(0,1);for(int i=1;i<count;i++){float a=activeRoad.pointFractions[i-1],b=activeRoad.pointFractions[i];if(motion[bus].fraction<=b||i==count-1){float u=b>a?constrain((motion[bus].fraction-a)/(b-a),0.f,1.f):0;x=point(i-1,0)+(point(i,0)-point(i-1,0))*u;y=point(i-1,1)+(point(i,1)-point(i-1,1))*u;break;}}int bob=motion[bus].fraction<.995f?((tick/650+bus)%2):0;nextX[nextCount]=constrain((int)roundf(x),145,296);nextY[nextCount]=constrain((int)roundf(y)+bob,48,161);nextAlpha[nextCount]=roundf(motion[bus].opacity*15);nextCount++;}
 if(nextCount==2)separateMarkerCenters(nextX[0],nextY[0],nextX[1],nextY[1]);
 bool unchanged=markerPainted&&markerCount==nextCount;for(int i=0;i<nextCount&&unchanged;i++)unchanged=markerX[i]==nextX[i]&&markerY[i]==nextY[i]&&markerAlpha[i]==nextAlpha[i];if(unchanged)return;if(markerPainted)restoreMarker();for(int i=0;i<nextCount;i++){markerX[i]=nextX[i];markerY[i]=nextY[i];markerAlpha[i]=nextAlpha[i];drawBusMarker(markerX[i],markerY[i],markerAlpha[i]);}markerCount=nextCount;markerPainted=nextCount>0;
}
