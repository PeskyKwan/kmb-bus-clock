...
bool applyConfig(JsonDocument&d,bool persist){
 ...
 cfg=next;cfg.gen++;alarmOn=false;etaEpoch=0;etaCode=0;dataStamp=0;needsDraw=true;lastPoll=0;failures=0;
...
void drawMain(){textBG=BG;lcd.fillScreen(BG);drawPlate();
 ...
 bool actualRoad=cfg.count==3&&!strcmp(cfg.stops[0].id,"1741D103CB826E93")&&!strcmp(cfg.stops[2].id,"5089C69E080B7A43");lcd.fillRoundRect(134,35,174,140,10,MAP);if(actualRoad){for(int y=0;y<155;y++)lcd.drawRGBBitmap(134,35+y,uiBackground+(35+y)*320+134,174,1);}textBG=MAP;double minLat=cfg.stops[0].lat,maxLat=minLat,minLng=cfg.stops[0].lng,maxLng=minLng;for(int i=1;i<cfg.count;i++){minLat=min(minLat,cfg.stops[i].lat);maxLat=max(maxLat,cfg.stops[i].lat);minLng=min(minLng,cfg.stops[i].lng);maxLng=max(maxLng,cfg.stops[i].lng);}double span=max(maxLat-minLat,(maxLng-minLng)*.925);if(span<.0001)span=.0001;int px=0,py=0;for(int i=0;i<cfg.count;i++){int x=157+(cfg.stops[i].lng-minLng)*.925/span*55,y=52+(maxLat-cfg.stops[i].lat)/span*95;if(actualRoad){x=186+810000*(cfg.stops[i].lng-114.2599)*PI/180.;y=107-810000*(log(tan(PI/4+cfg.stops[i].lat*PI/360))-log(tan(PI/4+22.3684*PI/360)));}if(i&&!actualRoad)for(int z=-1;z<=1;z++)lcd.drawLine(px+z,py,x+z,y,0xC4F1);if(!actualRoad){lcd.fillCircle(x,y,i==cfg.count-1?5:3,i==cfg.count-1?RED:INK);label(x+9,y-6,cfg.stops[i].name,12,INK,305-x-9);}px=x;py=y;}if(!actualRoad)label(272,150,"北↑",12);textBG=BG;lcd.fillRect(134,175,174,27,BG);label(136,185,actualRoad?"道路圖 · 未提供巴士定位":"站位示意 · 無定位",12);
