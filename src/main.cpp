...
bool api(const String&path,DynamicJsonDocument&out){for(int attempt=0;attempt<2;attempt++){WiFiClientSecure tls;tls.setCACert(kmbCA);tls.setHandshakeTimeout(12);tls.setTimeout(7);HTTPClient h;h.setConnectTimeout(6000);h.setTimeout(7000);h.useHTTP10(true);if(!h.begin(tls,"https://data.etabus.gov.hk/v1/transport/kmb/"+path))return false;int code=h.GET();char tlsError[1];int tlsCode=code<0?tls.lastError(tlsError,sizeof(tlsError)):0;apiTlsDiagnostic=tlsCode;if(code<0&&tlsCode==tlsConnectionEof&&!attempt){h.end();apiTlsRetries++;delay(300);continue;}bool ok=false;if(code==200&&h.getSize()<65536){DeserializationError err;if(path.startsWith("route-stop/")){StaticJsonDocument<160> filter;filter["data"][0]["stop"]=true;filter["data"][0]["seq"]=true;err=deserializeJson(out,h.getStream(),DeserializationOption::Filter(filter));}else err=deserializeJson(out,h.getStream());apiDiagnostic=(int)err.code();ok=!err;}else apiDiagnostic=code;h.end();return ok;}return false;}
