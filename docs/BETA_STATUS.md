# Installed second-arrival ETA —0.3.2 (2026-09-22)

The low-value route-window distance caption (for example `2.7km`) is replaced by `下一班 43分 · 估算`, showing the next distinct official target forecast beneath the map while the large countdown remains the first arrival. Both countdowns use the saved safety lead; raw official ETA values and bus-position matching remain unchanged. When the first arrival rolls over, the existing ordered forecast logic promotes the second and fills the following ETA; if no second forecast exists the line honestly shows `下一班 -- · 估算`. All11 host suites plus E32/CoreS3 builds pass. Personal0.3.2 binary2328224bytes, SHA256 `fb90d390ada85cb022446ff8899861ad1f27fa02148c09ae253fc27f7712ae0c`; app-only flash/hash verification passed with NVS preserved. Three live N241 samples retained route/Wi-Fi/settings, ETA code2, two illustrations and showed first/second display countdowns14/44 then13/43minutes. Lossless physical framebuffer visually confirms complete glyphs and an uncrowded `下一班 43分 · 估算` line. Not committed, pushed or published.

# Rotation full-frame retransmit fix —0.3.1 (2026-09-17)

Installed app-only after the first0.3.0 rotation implementation failed user review. Root cause: changing ST7789 rotation left the indexed renderer's tile hashes valid, so only logically changed tiles were resent and the physical panel mixed old/new orientation regions. `BufferedDisplay::invalidate()` now forces all300 16×16 tiles to retransmit after every orientation change. The strengthened device test requires `sentTiles` to increase by at least300 for normal→180° and again for180°→normal; both passed, with normal orientation restored and the90-second lead retained. This is transport-level physical-panel evidence, not a claim that Codex visually saw the inverted screen. Final personal binary2328848bytes, SHA256 `54eea9b4d7d68e610baafc164daa61dff87d79dbd695e781854873ad25567a7b`; app-only flash/hash, route/Wi-Fi/settings preservation and normal-orientation framebuffer pass. Not committed, pushed or published.

# Installed rotation and ETA safety lead —0.3.0 (2026-09-17)

Installed app-only at `0x10000`; esptool verified the written data hash and preserved NVS. Device settings add normal/180-degree orientation with matched calibrated-touch inversion and immediate persistence. Bus settings add0/30/60/90-second safety lead, default90seconds for existing configurations that lack the new field. The lead affects displayed countdown and one-shot alert timing only; raw KMB ETA, map matching, official timestamps and bus animation remain unchanged. A non-zero lead changes the main caption to `分鐘預留`. The original personal font assets were regenerated to include the new glyphs after framebuffer QA caught a missing `留`. All11 host suites, E32/CoreS3 builds, normal→inverted→normal settings smoke,90→60→90 save smoke, post-restart state and final physical framebuffer pass. Final personal binary2328832bytes, SHA256 `ad4a629e4f3108080a9ddc9751bbbdc9b9a3b04ce6e13cb0706621cc03ca5588`; no route/Wi-Fi/calibration loss, commit, push or publication.
