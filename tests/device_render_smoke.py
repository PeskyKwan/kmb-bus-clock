...
import serial
p=argparse.ArgumentParser();p.add_argument('--port',required=True);p.add_argument('--ui-only',action='store_true');a=p.parse_args();s=serial.Serial(port=None,baudrate=115200,timeout=.3);s.port=a.port;s.dtr=False;s.rts=False;s.open();time.sleep(10);s.reset_input_buffer()
...
try:
 ...
 while True:
  ...
  if a.ui_only and d.get('displayReady'):break
 ...
 ui('open');ui('touch',x=50,y=80);ui('touch',x=50,y=80);ui('touch',x=50,y=215);time.sleep(.5)
 ...
 ui('close');time.sleep(.5);before=state();time.sleep(20);after=state();delta=after['sentTiles']-before['sentTiles']
 if before.get('nightMode')!=after.get('nightMode'):
  print('Automatic theme transition:',delta,'tiles; palette change expected',flush=True)
 else:
  assert delta<300,delta
  print('Main-screen updates over20s:',delta,'tiles; no full-frame refresh PASS',flush=True)
finally:
 try:ui('close')
