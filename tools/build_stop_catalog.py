...
for s in rows:
 off=len(names);names.extend(s['name_tc'].encode()+b'\0');entries.append((int(s['stop'],16),off,round(float(s['lat'])*1e6),round(float(s['long'])*1e6)))
out=['#pragma once','#include <Arduino.h>','struct StopLabelEntry{uint64_t id;uint32_t offset;int32_t lat,lng;};','const StopLabelEntry stopLabels[] PROGMEM={']
out.extend('{0x%016xULL,%d,%d,%d},'%e for e in entries);out.append('};\nconst uint8_t stopLabelText[] PROGMEM={')
